...
create table stores(
  ...
  service_radius decimal(4,2) not null default 5.0,
...
create table refresh_tokens(
  ...
  foreign key (user_id) refrences employees(id) on delete cascade 
);
...
create table orders(
  ...
  foreign key (customer_id) references customers(id) on delete cascade,
  foreign key (store_id) references stores(id) on delete cascade,
  foreign key (delivery_boy_id) references delivery_boy(employee_id) on delete set null
);
...
create table product_alerts(
  ...
  foreign key (product_variant_id) references product_variants(id) on delete cascade,
  foreign key (store_id) references stores(id) on delete cascade,
  unique(customer_id,product_variant_id,store_id)
);
...
create table issues(
  ...
  issue_type enum('damaged','wrong_product','missing','expired','poor_quality','other') not null,
  description text,
  status enum(
